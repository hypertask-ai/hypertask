...
- For this mechanical check only, a valid `[BUGFIX]` or `[INFRA]` title may pass without a flag when the diff adds at most 150 lines to UI files. A verified auto-revert has the same exemption. These results still require owner merge and semantic review; a new feature cannot be disguised as a bug fix.
