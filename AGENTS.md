...
- Reviewers must block new feature or UI behavior pull requests that omit the required flag.
...
- A valid `[BUGFIX]` title passes the mechanical check without a flag regardless of UI line count. `[INFRA]` retains the 150-added-UI-line limit; verified auto-reverts retain their exemption. These results still require owner merge and semantic review; a new feature cannot be disguised as a bug fix.
