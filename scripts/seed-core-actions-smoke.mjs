...
const searchText = `core-actions-${runKey}`
...
try {
  ...
  const [sourceSection, targetSection] = await Promise.all([
    prisma.section.create({
      data: { projectId: project.id, section_title: 'Todo', ranking: 'A0100' },
    }),
    prisma.section.create({
      data: { projectId: project.id, section_title: 'Doing', ranking: 'A0200' },
    }),
  ])
  ...
  if (process.env.GITHUB_ACTIONS === 'true') {
    for (const value of [cookie, firebaseServiceAccount, betterAuthSecret, byokCipherSecret]) {
  ...
  const values = {
    ...
    CORE_SMOKE_TARGET_SECTION_ID: targetSection.id,
    CORE_SMOKE_TARGET_SECTION_TITLE: targetSection.section_title,
    CORE_SMOKE_SEARCH_TEXT: searchText,
  ...
  console.log(`Seeded core-actions fixture on board ${project.id}, task ${task.id}.`)
} finally {
