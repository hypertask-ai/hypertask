...
test("task-detail SQL selects public actors and sanitizes historical activity", () => {
  ...
  assert.equal(source.match(/\$\{hiddenCommentAgent\}/g)?.length, 3);
  assert.doesNotMatch(source, /hiddenCommentAgent\(/);
  ...
  assert.match(
    source,
    /sanitizeAgentCredentials\(\s*applyDurableAgentAttribution\(comments/,
  );
  assert.match(
    source,
