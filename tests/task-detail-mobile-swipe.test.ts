...
test("playlist bounds disable navigation at each end and outside the context", () => {
  ...
  assert.deepEqual(
    getTaskPlaylistBounds(playlist, { projectId: 15, uniqueIndex: 999 }),
    {
      currentIndex: -1,
      previousDisabled: true,
      nextDisabled: true,
...
test("release navigates in task order only past the threshold and within bounds", () => {
...
test("touches beginning in editors or horizontal scrollers never start a page swipe", () => {
  const dom = new JSDOM(`
    <main id="boundary">
      <div contenteditable="true"><span id="editor-target">text</span></div>
      <div id="scroller" style="overflow-x: auto"><button id="scroll-target">item</button></div>
      <div><button id="plain-target">item</button></div>
    </main>
  `);
  ...
  assert.equal(
    shouldIgnoreTaskSwipeStart(
      document.querySelector("#scroll-target"),
      boundary,
    ),
    true,
  );
  assert.equal(
    shouldIgnoreTaskSwipeStart(document.querySelector("#plain-target"), boundary),
    false,
  );
});
