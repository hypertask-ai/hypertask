...
5. **Check the flag's rollout state before judging the change.** Read the flag's mode at `https://app.hypertask.ai/admin/flags` (or `GET /api/admin/flags`, `listFeatureFlagModes` in `src/lib/flags.ts`). FAIL the ticket if the change is visible to a logged-in normal-user test account with the flag off, or if the flag's mode is anything other than `OWNER_AND_QA` (or narrower, e.g. `OWNER_ONLY`) without Valentin having explicitly widened it himself. `EVERYONE` is never a QA pass; name the flag key and its current mode in the FAIL comment.
6. **Compare against the acceptance criteria line by line.**
