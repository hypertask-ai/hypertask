...
3. **Trace the entry point.** Before editing, follow the change through middleware, route handler, controller or service layer, queue side effects, auth and cookie behaviour, and realtime/cache invalidation where relevant. Business logic usually lives in `src/utils/controllers/`, not in the route file. When monitoring discovers a fixture through one endpoint and consumes it through another, select it with the consumer's access and data constraints; ownership alone does not prove the acceptance checks can run.
4. **Write a failing test** that captures the reported behaviour. It must fail before your fix and pass after. It must exercise the real production implementation or its real external boundary, never a copied implementation plus string-matching the source. When QA supplies an exact failing command or request, test that same entry point and outcome; passing an adjacent route or related feature does not prove the ticket fixed. Match every data-state detail in the report: if the failure depends on a collection being populated, test it populated, because an empty-list case does not cover it. When a rule depends on a resource ever having entered a lifecycle state, move it out of that state before the forbidden action; current-state coverage is insufficient. For clipboard file bugs, drive the real paste handler with `DataTransfer.items` and `DataTransfer.files` independently because browsers may populate only one collection; assert that the event is consumed, content is inserted, and the original file reaches the upload boundary.
5. **Fix the bug** with the smallest diff that resolves it. Do not refactor unrelated code. For delegated authorization, scope every lookup and conflict check to the caller's authorized resources; an error must not reveal or apply account-wide data outside that boundary. See `ticket-lifecycle` "Acceptance criteria decide Done" for what to do with optional improvements. Cleanup jobs must identify resources owned by the current run, such as with a unique name, label, or network; never remove every resource sharing an image or type.
6. **Run the relevant tests** (not the whole suite unless the change is broad) on the same Node major CI uses, plus a fresh typecheck. Fix test loading through the repo's own test helpers; never convert a `.ts` file to `.js` just to satisfy an import. For monitoring and CI tickets, inspect the step outcomes and prove the acceptance checks executed; a green job whose checks skipped is not a pass. Mark the matching gate met with the test output as evidence: `$COMPANY_SKILLS_DIR/ticket-lifecycle/scripts/gates.sh met <PREFIX-NNN> "<gate>" --evidence "<test output>"` (`$COMPANY_SKILLS_DIR` is exported by the runner and points at the installed company-skills plugin, falling back to `~/projects/company-skills` when the plugin is not installed).
