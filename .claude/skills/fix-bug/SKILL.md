...
4. **Write a failing test** that captures the reported behaviour. It must fail before your fix and pass after. It must exercise the real production implementation or its real external boundary, never a copied implementation plus string-matching the source. Match every data-state detail in the report: if the failure depends on a collection being populated, test it populated, because an empty-list case does not cover it. When a rule depends on a resource ever having entered a lifecycle state, move it out of that state before the forbidden action; current-state coverage is insufficient. For clipboard file bugs, drive the real paste handler with `DataTransfer.items` and `DataTransfer.files` independently because browsers may populate only one collection; assert that the event is consumed, content is inserted, and the original file reaches the upload boundary.
