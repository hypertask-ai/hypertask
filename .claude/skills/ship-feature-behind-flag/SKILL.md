...
11. **Hand to QA only after the PR is merged and the production health check passes.** Name the flag key exactly in the ticket comment so QA can turn it on for their own account. An open PR or a preview is not ready for production QA. For a CLI change, a merge is not a release: verify the fleet-installed binary contains the merge and run the acceptance command through that installed binary before hand-off.
