...
10. **Expect the `feature-flag-gate` check.** It mechanically blocks a feature or UI PR that omits the flag. It lets a valid `[BUGFIX]` title through regardless of UI line count; `[INFRA]` retains the 150-added-UI-line limit and verified auto-reverts retain their exemption. API-only changes fall outside the mechanical check but the server-side rule in step 5 still applies, and no exemption waives semantic review.
