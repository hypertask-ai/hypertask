...
| What | URL | Status |
| --- | --- | --- |
| MCP server (Streamable HTTP) | `https://mcp.hypertask.ai/mcp` | live (401 without token; POST initialize / tools/list / tools/call are stateless on every instance; GET/DELETE session resume stays behind `htpr-6532-stateless-mcp`) |
