...
| `tests/core-actions-smoke-script.test.cjs` | The monitor runs every five minutes, serializes fixture use, limits rollback to production pushes, and provisions only from `production`. | GitHub evaluates schedules, concurrency, job conditions, and checkout refs before repository code runs. Executed tests separately cover rollback classification and reporting. |
| `tests/parity-contract.test.cjs` | Both trusted parity workflows pin the current CLI and one exact next version; the protected verifier also pins the next native release digest and verifies it before execution. | The workflow constants, command order, and package lock are the supply-chain trust boundary. The executable parity tests separately validate catalog policy and generated artifacts. |
