...
- The `production` ruleset requires `claude-review`, `pr-title`, `revert-guard`, `next-public-secrets`, and `secret-scan`. Auto-merge additionally requires `app-smoke` and `ci-tests` to be green; CI Tests normally reports after the pr-feedback timer applies the `full-ci` label on review success. The speed checks were retired with the 2026-08 staging rename; no `[SPEED]` gate exists today.
