...
- The required feature-flag gate also checks component reuse before it evaluates flags. A PR adding or renaming a JavaScript/TSX component file, or adding an inline menu, must map each changed exported control to a base-branch component binding it uses under **Components reused**, or give the structured no-reuse justification; tests, stories, and fixtures are excluded, and editing the PR body reruns the check.
...
- The `production` ruleset requires `app-smoke`, `ci-tests`, `claude-review`, `pr-title`, `revert-guard`, `next-public-secrets`, and `secret-scan`. CI Tests normally reports after the pr-feedback timer applies the `full-ci` label on review success. The speed checks were retired with the 2026-08 staging rename; no `[SPEED]` gate exists today.
