...
- The required feature-flag gate also checks component reuse before it evaluates flags. A PR adding a JavaScript/TSX component file or inline menu must map each triggering file to the base-branch component it imports under **Components reused**; tests, stories, and fixtures are excluded, and editing the PR body reruns the check.
