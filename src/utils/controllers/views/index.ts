...
import { canonicalBoardColumns, normalizeNativeBoardFilters } from '@/lib/mcp/views/nativeBoardSettings';
...
export * from "@/utils/controllers/projects/views/boardFilterWriteLock";
export * from "@/utils/controllers/projects/views/viewOrder";
export { default, default as getProjectView, getUniqueSlug, getViewUrl } from "@/utils/controllers/projects/views/viewsHelperAPIfunctions";

import type { CalendarDisplay, CalendarSavedView, CalendarSettings, CalendarSort, CalendarTaskFilters } from "@/models/Calendar/model";
import { extraMinimalProjectWhere } from "@/utils/controllers/projects/getAllMinimal";

export const calendarViewSelect = {
  id: true, createdAt: true, updatedAt: true, userId: true, title: true,
  visibility: true, projectIds: true, taskFilters: true, settings: true, sort: true,
} satisfies Prisma.Calendar_ViewSelect;

type CalendarViewRow = Prisma.Calendar_ViewGetPayload<{ select: typeof calendarViewSelect }>;

export const serializeCalendarView = (view: CalendarViewRow): CalendarSavedView => ({
  id: view.id,
...
export async function getAccessibleCalendarProjectIds(userId: number) {
...
) => projectIds.length > 0 && projectIds.every((projectId) => accessibleProjectIds.has(projectId));
