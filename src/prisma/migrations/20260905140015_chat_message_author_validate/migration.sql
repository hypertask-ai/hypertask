...
-- only ROW SHARE, which blocks nothing ordinary. Runs after the backfill,
-- whose single statement sets each column from a CASE on the message role, so
-- no row can end up with both authors set.
