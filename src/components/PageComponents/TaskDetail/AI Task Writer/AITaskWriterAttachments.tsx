...
const AITaskWriterAttachments: React.FC<AITaskWriterAttachmentsProps> = ({
  attachments,
  removeAttachment,
  className,
  isInHistory = false,
  ...
  const renderThumbnail = (attachment: IAIAttachment, index: number) => {
    switch (attachment.type) {
      case 'image':
        return (
          <div 
            className="p-1 grid bg-secondary h-[68px] w-[68px] overflow-y-hidden cursor-pointer hover:opacity-90 transition-opacity"
        ...
        {attachments.map((attachment, index) => (
          ...
          >
            <div className='ai-task-writer-attachment items-center w-full p-2 h-full flex flex-col rounded-md bg-[#27292D] text-white-black justify-end relative'>
              ...
              {/* File name */}
              <span className="ai-task-writer-attachment-name py-2 text-center text-white text-micro overflow-hidden w-[60px] line-clamp-3">
...
export default AITaskWriterAttachments;
