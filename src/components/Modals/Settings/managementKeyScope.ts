...
export const managementKeyTeamLabel = (
  ...
  if (teamScoped && !teamScopedKeysEnabled) return "Team key disabled";
  if (!teamScopedKeysEnabled) return null;
...
export const managementKeyScopeLabel = (
  ...
  const hasKnownResources = resourceNames.every(
    isKnownManagementPermissionResource,
  );
