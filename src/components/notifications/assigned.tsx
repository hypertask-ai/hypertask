...
const NotificationAssigned = (props: Props) => {
    ...
    return (
        <div id={`inbox-${notification.id}`}
         onClick={() => openTask()} 
         onMouseEnter={() => setSelectedInbox()} key={notification.id} className={`flex space-x-8 h-[70] py-2 px-4 rounded-md ${(_selectedInbox && _selectedInbox.id === notification.id) ? "bg-[#363A40]" : "transparent"} justify-between  w-full flex-col md:flex-row`}>
            <div className='flex space-x-6 w-1/5'>
                <span style={{ fontSize: 14, color: 'whire' }}>{notification.assignee?.assigner.displayName ?? ''}</span>
            </div>
            <div className='truncate flex-1 flex-column w-1/4'>
                <span style={{ fontSize: 14, color: 'whire' }}>{notification.assignee?.task?.title ?? ''}</span>
            </div>
            <div className='flex-1 flex-column w-1/4'>
                <span style={{ color: '#8E9093', fontSize: 14 }}>{'Assigned to you'}</span>
            </div>
            <span style={{ color: '#8E9093', fontSize: 14 }}>
...
export default NotificationAssigned
