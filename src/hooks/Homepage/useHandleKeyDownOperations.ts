...
const useHandleKeyDownOperations= (props:IHandleKeyDownOperations) => {
  ...
  //Updarte to useRead only recoil values or setOnly
    ...
    // debug.log("🚀 ~ file: Homepage.tsx:178 ~ handleKeyDown ~ document.activeElement?.id:", document.activeElement?.id)
    ...
    if (e.keyCode === 90&&undoData.length>0){
      // debug.log("🚀 ~ handleKeyDown ~ e.keyCode:", e.keyCode)
      const firstUndoData = undoData[undoData.length - 1];
      // debug.log("🚀 ~ handleKeyDown ~ firstUndoData:", firstUndoData)
  ...
  const CTRL_F_handler = (e:any)=>{
    ...
    useEffect(() => { 
        // debug.log("🚀 ~ useEffect ~ initialSections:", initialSections)
        ...
        // debug.log("🚀 ~ useEffect ~ _currentProject:", _currentProject)
        ...
        useEffect(() => {
            ...
            // debug.log("🚀 ~ useEffect ~ elementToFocus:")
            ...
            // debug.log('focus goes to ==>', firstTask.uniqueIndex)
  ...
  const onDragEndHandler = (result: DropResult) => {
    htLogger.time("onDragEndHandler"); // Start the timer
    const {source, destination, draggableId } = result;
    htLogger.info("🚀 ~ onDragEndHandler ~ result:", result)
    ...
    htLogger.info("🚀 ~ original Sections ~ sections:", sections)
    // return;
    ...
    htLogger.info("🚀 ~ item from original ~ filter:", filter)
    if (!filter || filter.length === 0) {
      htLogger.warn("🚀 ~ onDragEndHandler ~ dragged card not in rendered source column, aborting move:", draggableId);
      return;
    }
    ...
    const item = filter[0];
    htLogger.info("🚀 ~ item picked up: ~ item:", item)
    ...
    htLogger.info("🚀 ~ onDragEndHandler ~ newSections:", newSections)
    htLogger.timeEnd("onDragEndHandler"); // Stop the timer and log the elapsed time
