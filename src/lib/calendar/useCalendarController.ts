import { updateCalendarDueDate } from "./useCalendarSync";
import { shiftCalendarFocus } from "./useCalendarState";
import { dispatchCalendarCommand } from "./useCalendarDragDrop";
...
import { getFirstDayOfMonth, getLastDayOfMonth, getWeekStartIndex, startOfWeek, addMonths, subtractMonths, addDays, subtractDays, isTaskAssignedToMe, isSameDate } from "@/utils/helperFunctions/calendar.functions";
...
import { DEFAULT_CALENDAR_SETTINGS, DEFAULT_CALENDAR_TASK_FILTERS, DEFAULT_CALENDAR_VIEWS, sanitizeCalendarViewsPreference, type CalendarEverythingOverride, type CalendarSavedView, type CalendarViewCreateInput, type CalendarTaskFilters, type CalendarViewsOperation, type CalendarViewsPreference } from "@/models/Calendar/model";
import { fetchUserPreference, USER_PREFERENCES_QUERY_KEY, useGetUserPreferences, type IUserPreferences } from "@/hooks/General/useGetUserPreferences";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import { useSyncedCalendarReadModel } from "@/hooks/Calendar/useSyncedCalendarReadModel";
...
import { clearCalendarSessionDraft, readCalendarSessionDraft, resolveCalendarSessionDraft, writeCalendarSessionDraft } from "@/lib/calendarSessionDraft";
...
const normalizedTaskFilters = (filters: CalendarTaskFilters): CalendarTaskFilters => ({
...
type CalendarViewState = Pick<CalendarSavedView, "checkedProjects" | "taskFilters" | "settings" | "sort">;
const canonicalCalendarViewState = (state: CalendarViewState): CalendarViewState => ({
...
const buildCalendarTaskFilterSets = (filters: CalendarTaskFilters): CalendarTaskFilterSets => ({
...
export function useCalendarView(accountId: number) {
    ...
    const { tasks: allData, projects, isPending: isCalendarDataPending, hasError: calendarDataError, retry: retryCalendarData, reconcile: reconcileCalendar, updateTaskProjection, } = useSyncedCalendarReadModel({
        accountId,
        currentDate,
        currentView,
        weekStartsOn: calendarSettings.weekStartsOn,
    });
    const [currentDay, setCurrentDay] = useState<Date>(new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate()));
    ...
    useEffect(() => {
        ...
        params.set("date", `${currentDate.getFullYear()}-${pad(currentDate.getMonth() + 1)}-${pad(currentDate.getDate())}`);
        window.history.replaceState(null, "", `${window.location.pathname}?${params.toString()}`);
    }, [currentView, currentDate]);
    const { updateTaskInCache } = UpdateKanban();
    const { updateActiveItemAndItemInView, goToProjectShortcut } = useProjectQuery();
    ...
    const [showManageTasksModal, setShowManageTasksModal] = useState<{
        show: boolean;
        date: Date;
    } | undefined>(undefined);
    ...
    const createTaskModal = useRecoilValue(showCreateTaskModalAtom);
    const [checkedProjects, setCheckedProjects] = useRecoilState(calendarCheckedProjectsAtom);
    const [taskFilters, setTaskFilters] = useRecoilState(calendarTaskFiltersAtom);
    ...
    const calendarViewsQueryKey = useMemo(() => [...CALENDAR_VIEWS_QUERY_KEY, accountId] as const, [accountId]);
    ...
    const calendarViewsPreference = useMemo(() => sanitizeCalendarViewsPreference(userPreferencesQuery.data.calendarViews) ?? DEFAULT_CALENDAR_VIEWS, [userPreferencesQuery.data.calendarViews]);
    const everythingTitle = calendarViewsPreference.everything?.title ?? "Everything";
    const calendarViews = useMemo(() => [...calendarViewsQuery.data].sort((left, right) => {
    ...
    everything: CalendarEverythingOverride | null = null) => {
        ...
        setCheckedProjects(Object.fromEntries(state.checkedProjects.map((projectId) => [projectId, true])));
    ...
    const applyStoredCalendarViewState = useCallback((value: unknown) => {
        const preference = sanitizeCalendarViewsPreference(value) ?? DEFAULT_CALENDAR_VIEWS;
        const appliedView = calendarViews.find((view) => view.id === preference.appliedViewId);
        applyCalendarViewState(appliedView ?? null, preference.everything ?? null);
    }, [applyCalendarViewState, calendarViews]);
    const initialCalendarViewState = useRef(JSON.stringify({
        checkedProjects,
        taskFilters,
        calendarSettings,
        currentView,
    ...
    useEffect(() => {
        if (hasHydratedCalendarViews.current)
            return;
        ...
        if (initialCalendarViewState.current === nextState)
            return;
    ...
    useEffect(() => {
        ...
        if (hasInteractedWithCalendarState.current)
            return;
        const sessionDraft = resolveCalendarSessionDraft(readCalendarSessionDraft(window.sessionStorage, accountId), appliedCalendarViewId);
        if (sessionDraft) {
        ...
        const appliedView = calendarViews.find((view) => view.id === calendarViewsPreference.appliedViewId);
        if (appliedView)
            applyCalendarViewState(appliedView);
    ...
    const persistCalendarViews = useCallback(async (operation: CalendarViewsOperation, calendarViews: CalendarViewsPreference) => {
        const writeVersion = ++calendarViewsWriteVersion.current;
        queryClient.setQueryData<IUserPreferences>(USER_PREFERENCES_QUERY_KEY, (current) => current ? { ...current, calendarViews } : current);
        try {
            ...
            if (writeVersion === calendarViewsWriteVersion.current) {
                queryClient.setQueryData<IUserPreferences>(USER_PREFERENCES_QUERY_KEY, response.data.settings);
                if (calendarViewsNeedServerState.current) {
                    applyStoredCalendarViewState(response.data.settings.calendarViews);
    ...
    const setCurrentViewFromInteraction: typeof setCurrentView = useCallback((value) => {
        hasInteractedWithCalendarState.current = true;
    ...
    const setCalendarSortFromInteraction: typeof setCalendarSort = useCallback((value) => {
        hasInteractedWithCalendarState.current = true;
    ...
    const applyCalendarView = useCallback(async (viewId: string | null) => {
        hasInteractedWithCalendarState.current = true;
        const view = calendarViews.find((item) => item.id === viewId) ?? null;
        applyCalendarViewState(view, calendarViewsPreference.everything ?? null);
        await persistCalendarViews({ type: "setAppliedViewId", appliedViewId: view?.id ?? null }, {
            ...calendarViewsPreference,
    ...
    const saveCalendarView = useCallback(async (title: string, updateApplied: boolean, visibility: ViewVisibility) => {
        const appliedView = calendarViews.find((view) => view.id === appliedCalendarViewId);
        if (updateApplied && !appliedView) {
            const trimmedTitle = title.trim();
            if (trimmedTitle.length > 60)
                return false;
            const previousTitle = calendarViewsPreference.everything?.title?.trim();
            ...
            return persistCalendarViews({ type: "setEverything", everything }, { ...calendarViewsPreference, everything });
        }
        const trimmedTitle = title.trim();
        if (!trimmedTitle || trimmedTitle.length > 60)
            return false;
        const savedVisibility = updateApplied && appliedView ? appliedView.visibility : visibility;
        const input: CalendarViewCreateInput = {
            title: trimmedTitle,
            visibility: savedVisibility,
            projectIds: materializeCalendarViewProjectIds(savedVisibility, currentCalendarView.checkedProjects, projects.map((project) => project.id)),
        ...
        try {
            if (updateApplied && appliedView) {
                const response = await axios.patch(`/api/calendar/views/${appliedView.id}`, { ...input, visibility: appliedView.visibility });
                queryClient.setQueryData<CalendarSavedView[]>(calendarViewsQueryKey, (current) => current?.map((view) => view.id === appliedView.id ? response.data.view : view) ?? [response.data.view]);
            ...
            queryClient.setQueryData<CalendarSavedView[]>(calendarViewsQueryKey, (current) => [...(current ?? []), createdView]);
            try {
                await queryClient.invalidateQueries({ queryKey: calendarViewsQueryKey });
            }
            catch (error) {
            ...
            const applied = await persistCalendarViews({ type: "setAppliedViewId", appliedViewId: createdView.id }, { ...calendarViewsPreference, appliedViewId: createdView.id });
            return applied;
        }
        catch (error) {
            toast.error(axios.isAxiosError(error) &&
    ...
    const renameCalendarView = useCallback(async (viewId: string, title: string) => {
        const trimmedTitle = title.trim();
        if (!trimmedTitle || trimmedTitle.length > 60)
            return false;
        const renamedView = calendarViews.find((view) => view.id === viewId);
        if (!renamedView)
            return false;
        try {
            ...
            queryClient.setQueryData<CalendarSavedView[]>(calendarViewsQueryKey, (current) => current?.map((view) => view.id === viewId ? response.data.view : view) ?? [response.data.view]);
            await queryClient.invalidateQueries({ queryKey: calendarViewsQueryKey });
            return true;
        }
        catch (error) {
    ...
    const renameEverything = useCallback(async (title: string) => {
        const trimmedTitle = title.trim();
        if (!trimmedTitle || trimmedTitle.length > 60)
            return false;
        const everything: CalendarEverythingOverride = {
            ...canonicalCalendarViewState(calendarViewsPreference.everything ?? DEFAULT_CALENDAR_VIEW_STATE),
            title: trimmedTitle,
        };
        return persistCalendarViews({ type: "setEverything", everything }, { ...calendarViewsPreference, everything });
    }, [calendarViewsPreference, persistCalendarViews]);
    const deleteCalendarView = useCallback(async (viewId: string) => {
        const deletingApplied = appliedCalendarViewId === viewId;
        try {
            await axios.delete(`/api/calendar/views/${viewId}`);
            queryClient.setQueryData<CalendarSavedView[]>(calendarViewsQueryKey, (current) => current?.filter((view) => view.id !== viewId) ?? []);
            await queryClient.invalidateQueries({ queryKey: calendarViewsQueryKey });
            if (!deletingApplied)
                return true;
            applyCalendarViewState(null, calendarViewsPreference.everything ?? null);
            return persistCalendarViews({ type: "setAppliedViewId", appliedViewId: null }, { ...calendarViewsPreference, appliedViewId: null });
        }
        catch (error) {
            toast.error(axios.isAxiosError(error) &&
                typeof error.response?.data?.error === "string"
    ...
    const isAssigneeAgentsFilter = useCallback((task: ITask, assigneeAgents: ReadonlySet<string>): boolean => {
        if (assigneeAgents.size === 0)
            return false;
        return task.assignees?.some((assignee) => assignee.agentId != null && assigneeAgents.has(assignee.agentId)) ?? false;
    }, []);
    const isUpdatedByAgentsFilter = useCallback((task: ITask, updatedByAgents: ReadonlySet<string>): boolean => {
        if (updatedByAgents.size === 0)
            return false;
    ...
    const taskFilterSets = useMemo(() => buildCalendarTaskFilterSets(taskFilters), [taskFilters]);
    ...
    const tasks = useMemo(() => {
        ...
        const projectFilteredTasks: ITask[] = hasSelectedProjects
            ? allData.filter((task) => task.projectId != null && checkedProjects[task.projectId] === true)
            : [...allData];
        const hasActiveTaskFilters = Object.values(taskFilters).some(Boolean);
        if (hasActiveTaskFilters) {
            return projectFilteredTasks.filter((task) => taskMatchesFilters(task, taskFilters, taskFilterSets));
        }
        return projectFilteredTasks;
    }, [allData, checkedProjects, taskFilters, taskFilterSets, taskMatchesFilters]);
    const calendarSortComparator = useMemo(() => calendarSort
    ...
    const { weeks } = useMemo(() => {
        ...
        const startDay = getWeekStartIndex(monthStart, calendarSettings.weekStartsOn);
        ...
        if (currentView === "month") {
            ...
            for (let i = 1; i < 7 - endDay; i++) {
                daysArray.push(new Date(monthEnd.getFullYear(), monthEnd.getMonth() + 1, i));
            }
            ...
            const weekStart = startOfWeek(currentDate, calendarSettings.weekStartsOn);
            for (let i = 0; i < 7; i++) {
    ...
    const { calendarViewCounts, visibleTaskCount } = useMemo(() => {
        const visibleDateKeys = new Set(weeks.flat().map((date) => `${date.getFullYear()}-${date.getMonth()}-${date.getDate()}`));
        const visibleTasks = allData.filter((task) => {
            if (!task.dueDate)
                return false;
            const date = new Date(task.dueDate);
            return visibleDateKeys.has(`${date.getFullYear()}-${date.getMonth()}-${date.getDate()}`);
        });
        ...
        for (const view of calendarViews) {
            ...
            counts.set(view.id, visibleTasks.filter((task) => (!hasSelectedProjects ||
                (task.projectId != null &&
                    selectedProjects.has(task.projectId))) &&
                taskMatchesFilters(task, view.taskFilters, filterSets)).length);
        }
        return {
    ...
    const setCurrentTaskWithFocus = useCallback((taskId: number, day?: Date) => {
        ...
        if (taskId === FOCUS_MORE_INDICATOR && day) {
            const moreElement = document.getElementById(calendarConfig.element_ids.more_tasks_indicator(day));
            if (moreElement) {
                moreElement.focus();
        ...
        if (taskId !== -1) {
            ...
            const dayElement = document.getElementById(`day-${dayToScroll.toISOString()}`);
            if (dayElement) {
    ...
    const updateFocusAfterNavigation = (newDate: Date) => {
        ...
        if (currentView === "month") {
            ...
            // Add days of current month
            for (let i = 1; i <= monthEnd.getDate(); i++) {
                daysInNewRange.push(new Date(newDate.getFullYear(), newDate.getMonth(), i));
            }
            // Add days from next month
            for (let i = 1; i < 7 - endDay; i++) {
                daysInNewRange.push(new Date(monthEnd.getFullYear(), monthEnd.getMonth() + 1, i));
            }
        }
        ...
        if (dayToFocus) {
            ...
            const dayElement = document.getElementById(`day-${dayToFocus!.toISOString()}`);
            if (dayElement) {
                dayElement.focus();
                dayElement.scrollIntoView({
                    behavior: "smooth",
    ...
    const updateDueDateHandler = useCallback((task: ITask, dueDate: Date | undefined) => updateCalendarDueDate(task, dueDate, () => ({ currentDay, currentTask, getDefaultOptions, getTasksForDate, onTaskUpdate, setCurrentDay, setCurrentTaskWithFocus })), [getDefaultOptions, currentTask, currentDay, getTasksForDate, setCurrentTaskWithFocus, onTaskUpdate]);
    const addTaskToState = useCallback(async (task: ITask, date: Date) => {
        try {
            ...
            if (response.status === 200) {
                ...
                updateTaskInCache(updatedTask, updatedTask.id, updatedTask.projectId, updatedTask.sectionId);
    ...
    const toggleDueDateModal = useCallback((mode?: "Create" | "Update", payload?: {
        task?: ITask;
        date: Date | undefined;
    }) => {
        if (!mode) {
            ...
            if (currentMode === "Update")
                dueDateModalCallback(date || undefined, task);
            else if (currentMode === "Create" && task && date)
                addTaskToState(task, date || undefined);
        }
        else
            setShowDueDateModal({ show: true, mode });
    ...
    const handleTaskClick = useCallback((taskDay: Date, task: ITask) => {
        ...
        setTaskPlayList(tasks.map((task) => ({
    ...
    const toggleManageTasksModal = useCallback((date: Date, close?: boolean, mode?: "Add" | "Update" | "Visit", task?: ITask) => {
    ...
    function handleDateSelect(date: Date | undefined) {
        try {
            ...
            let normalizedDate = new Date(date.getFullYear(), date.getMonth(), date.getDate());
            if (currentView === "week" &&
                !calendarSettings.showWeekends &&
                (normalizedDate.getDay() === 0 || normalizedDate.getDay() === 6)) {
                normalizedDate = startOfWeek(normalizedDate, calendarSettings.weekStartsOn);
            ...
            const dateExistsInView = allDaysInView.some((day) => isSameDate(day, normalizedDate));
            if (dateExistsInView) {
                // Date is in current view, focus on it immediately
                setCurrentDay(normalizedDate);
                // Focus on the day element
                const dayElement = document.getElementById(`day-${normalizedDate.toISOString()}`);
                if (dayElement) {
                    dayElement.focus();
                    dayElement.scrollIntoView({
                        behavior: "smooth",
                ...
                setCurrentTaskWithFocus(tasksForDate.length > 0 ? tasksForDate[0].id : -1, normalizedDate);
            }
            else {
    ...
    const shiftFocusHorizontally = useCallback((direction: "left" | "right") => {
        try {
            ...
            const currentDayIndex = daysInView.findIndex((day) => isSameDate(day, currentDay));
    ...
    const shiftFocusVertically = useCallback((direction: "up" | "down") => shiftCalendarFocus(direction, () => ({ FOCUS_MORE_INDICATOR, currentDay, currentTask, currentView, getTasksForDate, setCurrentDay, setCurrentTaskWithFocus, tasks, weeks })), [currentDay, currentTask, currentView, weeks, getTasksForDate, setCurrentTaskWithFocus]);
    ...
    const moveTaskVertically = useCallback(async (direction: "up" | "down") => {
        try {
            ...
            if (direction === "up" && currentWeekIndex === 0)
                return;
            if (direction === "down" && currentWeekIndex === weeks.length - 1)
                return;
            // Get the current task
            const currentTasks = getTasksForDate(currentDay);
            const task = currentTasks.find((t) => t.id === currentTask);
            ...
            const newDueDate = new Date(currentDay.getFullYear(), currentDay.getMonth(), currentDay.getDate() + weekOffset);
            await updateDueDateHandler(task, newDueDate);
        }
        catch (error) {
    ...
    async function onDragEnd(result: DropResult) {
        try {
            ...
            const destinationElement = document.getElementById(result.destination.droppableId);
    ...
    useEffect(() => {
        ...
        const allDaysInView = weeks.flat();
        const currentDayExists = allDaysInView.some((day) => isSameDate(day, currentDay));
    ...
    useEffect(() => {
        if (!hasInitializedTaskRef.current &&
            tasksByDate.size > 0 &&
            currentTask === -1) {
    ...
    useEffect(() => {
        if (pendingDateSelectRef.current) {
            ...
            const allDaysInView = weeks.flat();
            const dateExistsInView = allDaysInView.some((day) => isSameDate(day, targetDate));
            if (dateExistsInView) {
                // Date is now in view, focus on it
                setCurrentDay(targetDate);
                // Focus on the day element
                const dayElement = document.getElementById(`day-${targetDate.toISOString()}`);
                if (dayElement) {
                    dayElement.focus();
                    dayElement.scrollIntoView({
                        behavior: "smooth",
                ...
                setCurrentTaskWithFocus(tasksForDate.length > 0 ? tasksForDate[0].id : -1, targetDate);
