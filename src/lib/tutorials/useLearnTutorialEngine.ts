import { dismissTutorialTaskModal, dispatchTutorialEscape, getRenderedBoardPosition, getRenderedTutorialInboxNotificationIds, getRenderedTutorialInboxTargetAtIndex, parseTutorialInboxTargets } from "./tutorialSteps";
"use client";
import { useCallback, useEffect, useRef, useState } from "react";
import { usePathname, useRouter, useSearchParams } from "next/navigation";
import { useRecoilValue, useResetRecoilState } from "@/lib/state";
import { activeItemAtom, currentUserAtom, globalNotificationFocusAtom, showBoardManagerAtom, showCommandsAtom, showShortcutsAtom } from "@/store";
import { activateLearnTutorialForRequest, advanceLearnTutorialForPath, canOpenLearnTutorialTask, createLearnTutorialState, getLearnTutorialStorageKey, isLearnTutorialEligiblePath, LEARN_TUTORIAL_COMMENT_SAVED_EVENT, LEARN_TUTORIAL_COLUMN_CREATED_EVENT, LEARN_TUTORIAL_COLUMN_MOVE_EVENT, LEARN_TUTORIAL_DUE_DATE_SAVED_EVENT, LEARN_TUTORIAL_INBOX_ARCHIVED_EVENT, LEARN_TUTORIAL_INBOX_ARCHIVE_FAILED_EVENT, LEARN_TUTORIAL_STORAGE_KEY, LEARN_TUTORIAL_STATE_UPDATED_EVENT, LEARN_TUTORIAL_TASK_MOVED_EVENT, observeLearnTutorialBoardMove, observeLearnTutorialColumnCreated, observeLearnTutorialColumnMove, observeLearnTutorialEscape, observeLearnTutorialInboxArchive, observeLearnTutorialInboxFocus, observeLearnTutorialInboxNavigation, observeLearnTutorialInboxTaskOpen, observeLearnTutorialInboxZero, observeLearnTutorialTaskFocus, observeLearnTutorialSurface, parseLearnTutorialState, startLearnTutorialMovement, type LearnTutorialMovementKey, type LearnTutorialBoardMoveDirection, type LearnTutorialBoardPosition, type LearnTutorialColumnCreatedDetail, type LearnTutorialColumnMoveDetail, type LearnTutorialInboxArchivedDetail, type LearnTutorialInboxTarget, type LearnTutorialSurface, type LearnTutorialTaskMovedDetail, type LearnTutorialTaskPersistedDetail } from "@/lib/tutorial/learnTutorialState";
import { learnTutorialScenes, type LearnTutorialScene } from "@/hooks/General/useScenesV2";
export const useLearnTutorial = () => {
    ...
    const requestedReturnBoardId = Number.isSafeInteger(tutorialReturnParam) && tutorialReturnParam > 0
        ? tutorialReturnParam
        : null;
    const requestedTutorialInboxTargets = parseTutorialInboxTargets(searchParams?.get("tutorialInbox") ?? null);
    ...
    const [tutorialState, setTutorialState] = useState(() => createLearnTutorialState(false));
    ...
    const pendingMovementClearTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
    const pendingSurface = useRef<LearnTutorialSurface | null>(null);
    const pendingSurfaceGeneration = useRef(0);
    const pendingSurfaceClearTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
    const pendingSurfaceReveal = useRef<LearnTutorialSurface | null>(null);
    const pendingSurfaceRevealTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
    ...
    const pendingInboxMovementTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
    const pendingInboxArchiveTarget = useRef<LearnTutorialInboxTarget | null>(null);
    ...
    const inboxBootstrapRetryTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
    ...
    const pendingBoardMoveTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
    const pendingBoardMoveVerificationTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
    const [commandHandoffGeneration, setCommandHandoffGeneration] = useState<number | null>(null);
    ...
    const expectSurface = useCallback((surface: LearnTutorialSurface, timeout = 1600) => {
        clearPendingSurface();
        pendingSurface.current = surface;
        pendingSurfaceClearTimer.current = setTimeout(clearPendingSurface, timeout);
    }, [clearPendingSurface]);
    const revealSurfaceAfterShortcut = useCallback((surface: LearnTutorialSurface, opened: () => boolean, afterReveal?: () => void) => {
        const expectationGeneration = pendingSurfaceGeneration.current;
        pendingSurfaceReveal.current = surface;
        const waitForOpen = () => {
            const expectationCurrent = pendingSurface.current === surface &&
                pendingSurfaceGeneration.current === expectationGeneration;
            if (!expectationCurrent)
                return;
            if (!opened()) {
            ...
            pendingSurfaceRevealTimer.current = setTimeout(() => {
                pendingSurfaceReveal.current = null;
                pendingSurfaceRevealTimer.current = null;
                const shortcutMatched = pendingSurface.current === surface &&
                    pendingSurfaceGeneration.current === expectationGeneration &&
                    opened();
                if (!shortcutMatched)
                    return;
                setTutorialState((state) => observeLearnTutorialSurface(state, surface, true, true));
                clearPendingSurface();
                afterReveal?.();
            }, 700);
        };
    ...
    const expectBoardMove = useCallback((direction: LearnTutorialBoardMoveDirection, taskId: number | null) => {
        ...
        pendingBoardMoveTimer.current = setTimeout(clearPendingBoardMove, 20000);
        return true;
    }, [clearPendingBoardMove]);
    useEffect(() => {
        resetInboxBootstrap();
        ...
        const stored = parseLearnTutorialState(window.sessionStorage.getItem(storageKey));
    ...
    useEffect(() => {
        if (!hydrated || !tutorialEligible)
            return;
        setTutorialState((state) => {
            const activated = activateLearnTutorialForRequest(state, tutorialRequested);
    ...
    useEffect(() => {
        ...
        const candidateTargets = !inboxBootstrapIgnoreCandidates.current &&
        ...
        void fetch("/api/learn/board", {
            ...
            body: JSON.stringify({
                ...(candidateTargets.length === 2
                    ? {
                        tutorialInboxArchivedNotificationIds: tutorialState.archivedInboxNotificationIds,
            ...
            if (!response.ok ||
                ...
                tutorialInboxTargets.some((target) => !Number.isSafeInteger(target.notificationId) ||
            ...
            if (!inboxBootstrapMounted.current)
                return;
            ...
            setTutorialState((state) => ({
                ...
                tutorialColumnTitle: state.tutorialColumnId !== null &&
    ...
    useEffect(() => {
        ...
        const updateOpenSurfaces = () => {
            const next = {
                aiWriter: Boolean(document.querySelector("#popover-wrapper-description textarea#htc")),
                ...
                commentEditor: Boolean(document.querySelector('#comment-input [contenteditable="true"]') &&
                    document.activeElement?.closest("#comment-input")),
            };
            setOpenSurfaces((current) => Object.keys(next).every((key) => next[key as keyof typeof next] ===
    ...
    useEffect(() => {
        ...
        const updateKeyword = (event: Event) => {
            ...
            if (input?.id === "filter-input")
                setDueDateKeyword(input.value.trim());
        };
    ...
    const exitTutorial = useCallback(() => {
        ...
        resetShowShortcuts();
        setTutorialState(createLearnTutorialState(false));
        router.push(tutorialState.returnBoardId !== null &&
    ...
    const continueTutorial = useCallback(() => {
        const firstTask = document.querySelector<HTMLElement>("#sectionsContainer [id^='task-']");
        const firstTaskId = Number(firstTask?.id.slice("task-".length));
        if (!firstTask || !Number.isSafeInteger(firstTaskId) || firstTaskId <= 0)
            return;
        firstTask.focus();
        setTutorialState((state) => state.scene === "welcome"
            ? startLearnTutorialMovement(state, firstTaskId)
            : state);
    }, []);
    const archiveTutorialInboxTarget = useCallback(async (target: LearnTutorialInboxTarget, projectId: number) => {
        try {
            const response = await fetch(`/api/notifications/markAsDone?id=${target.notificationId}&taskId=${target.taskId}&tutorial=1`, { method: "GET" });
            if (!response.ok) {
            ...
            window.dispatchEvent(new CustomEvent(LEARN_TUTORIAL_INBOX_ARCHIVED_EVENT, { detail }));
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            if ((tutorialState.scene === "actTwoComplete" ||
            ...
            if (tutorialState.scene === "boardSwitcher" &&
                commandPressed &&
                key === "b" &&
                !event.altKey &&
                !event.shiftKey) {
                expectSurface("board-switcher", 5000);
                revealSurfaceAfterShortcut("board-switcher", () => Boolean(document.getElementById("boardManager")), resetShowBoardManager);
                setPressedKey("b");
                return;
            }
            if (tutorialState.scene === "addColumnCommand" &&
                commandPressed &&
                key === "k" &&
                !event.altKey &&
                !event.shiftKey) {
                expectSurface("command-center", 5000);
                revealSurfaceAfterShortcut("command-center", () => Boolean(document.querySelector("input#htc")));
                setPressedKey("k");
                return;
            }
            const activeInput = document.activeElement as HTMLInputElement | null;
            if (tutorialState.scene === "addColumnSearch") {
                const commandInput = activeInput?.id === "htc" &&
                ...
                if (event.key === "Enter" &&
                    ...
                    expectSurface("add-column", 5000);
                    revealSurfaceAfterShortcut("add-column", () => Boolean(document.getElementById("addColumnModal")));
                    setPressedKey("enter");
                    return;
                }
            ...
            if (tutorialState.scene === "addColumnName") {
                const columnInput = document.querySelector<HTMLInputElement>("#addColumnModal input");
                if (event.key === "Enter" &&
                    columnInput !== null &&
            ...
            if (tutorialState.scene === "moveTaskCommand" &&
                key === "m" &&
                !commandPressed &&
                !event.altKey &&
                !event.shiftKey &&
                ...
                expectSurface("move-to-column", 5000);
                revealSurfaceAfterShortcut("move-to-column", () => Boolean(document.getElementById("MoveModal")));
                setPressedKey("m");
                return;
            }
            if (tutorialState.scene === "moveTaskPick") {
                const moveInput = document.querySelector<HTMLInputElement>("#MoveModal input#linksModal");
                if (event.key === "Enter" &&
                    moveInput !== null &&
            ...
            if (tutorialState.scene === "shortcutsRecap" &&
                event.code === "Slash" &&
                event.shiftKey &&
                !commandPressed &&
                !event.altKey) {
                expectSurface("keyboard-shortcuts", 5000);
                revealSurfaceAfterShortcut("keyboard-shortcuts", () => Boolean(document.getElementById("keyboard-shortcut-container")), resetShowShortcuts);
                setPressedKey("?");
                return;
            }
            if (tutorialState.scene === "finale" &&
                event.key === "Enter" &&
                !commandPressed &&
                !event.altKey &&
                !event.shiftKey) {
            ...
            if (tutorialState.scene === "goInbox" &&
                ...
                if (key === "g") {
                    ...
                    setPressedKey("g");
                    setTimeout(() => {
                        if (pendingInboxGAt.current !== startedAt)
                            return;
                ...
                if (key === "i" &&
                    pendingInboxGAt.current !== null &&
                    Date.now() - pendingInboxGAt.current < 1500) {
                    event.preventDefault();
                    event.stopImmediatePropagation();
                    ...
                    pendingInboxGAt.current = null;
                    setPressedKey("i");
                    router.push(`/inbox?projectId=${tutorialState.learnBoardId}&tutorial=1`);
                    return;
                }
            }
            if (tutorialState.scene === "inboxTriage" &&
                !commandPressed &&
                !event.altKey &&
                !event.shiftKey &&
                ...
                if (key === "j" || key === "k") {
                    ...
                    pendingInboxMovementTimer.current = setTimeout(clearPendingInboxMovement, 2000);
                    setPressedKey(key);
                    return;
                }
                const target = getRenderedTutorialInboxTargetAtIndex(inboxFocus.currIdx);
                if (target === null) {
                    event.preventDefault();
                    event.stopImmediatePropagation();
                    return;
            ...
            if (tutorialState.scene === "openInboxTask" &&
                event.key === "Enter" &&
                !commandPressed &&
                !event.altKey &&
                !event.shiftKey) {
            ...
            if (tutorialState.scene === "inboxTaskArchive" &&
                key === "e" &&
                !commandPressed &&
                !event.altKey &&
                !event.shiftKey) {
                ...
                const target = tutorialState.lastInboxNotificationId !== null &&
                    ...
                    : null;
                if (target === null || tutorialState.learnBoardId === null)
                    return;
                pendingInboxArchiveTarget.current = target;
                setPressedKey("e");
            ...
            if (tutorialState.scene === "aiWriter" &&
                commandPressed &&
                key === "j" &&
                !event.altKey &&
                !event.shiftKey) {
            ...
            if (tutorialState.scene === "commandCenter" &&
                commandPressed &&
                key === "k" &&
                !event.altKey &&
                !event.shiftKey) {
            ...
            if (tutorialState.scene === "priority" &&
                key === "p" &&
                !event.altKey &&
                !commandPressed &&
                !event.shiftKey) {
            ...
            if (tutorialState.scene === "dueDate" &&
                key === "d" &&
                !event.altKey &&
                !commandPressed &&
                !event.shiftKey) {
            ...
            if (tutorialState.scene === "dueDateTomorrow" &&
                event.key === "Enter" &&
                document.activeElement?.id === "filter-input" &&
                (document.activeElement as HTMLInputElement).value
                    .trim()
                    .toLowerCase() === "tomorrow") {
                expectSurface("due-date-committed", 20000);
                setPressedKey("enter");
                return;
            }
            if (tutorialState.scene === "comment" &&
                commandPressed &&
                key === "m" &&
                !event.altKey &&
                !event.shiftKey) {
            ...
            if (tutorialState.scene === "commentSave" &&
                commandPressed &&
                event.key === "Enter" &&
                !event.altKey &&
                !event.shiftKey &&
                ...
                expectSurface("comment-saved", 20000);
                setPressedKey("enter");
                return;
            }
            if (tutorialState.scene === "escape" && event.key === "Escape") {
                pendingEscapeFromDetail.current = (pathname ?? "").startsWith("/detail/");
            ...
            const expectedBoardMove = tutorialState.scene === "moveAcross"
                ...
                : tutorialState.scene === "reorder"
                    ...
                    : null;
            const tutorialTaskFocused = tutorialState.lastTaskId !== null &&
                document.activeElement?.id === `task-${tutorialState.lastTaskId}`;
            if (expectedBoardMove &&
                tutorialTaskFocused &&
                event.shiftKey &&
                !event.altKey &&
                !commandPressed &&
                ...
                if (!expectedBoardMove.keys.includes(key) ||
                    event.repeat ||
                    pendingBoardMove.current !== null) {
                    event.preventDefault();
                    event.stopImmediatePropagation();
                ...
                if (!expectBoardMove(expectedBoardMove.direction, tutorialState.lastTaskId)) {
                    event.preventDefault();
                    event.stopImmediatePropagation();
                    return;
                }
            ...
            if (tutorialState.scene !== "movement")
                return;
            if ((key === "j" || key === "k") &&
                !event.altKey &&
                !event.ctrlKey &&
        ...
        const handleKeyUp = (event: KeyboardEvent) => {
            ...
            const releasedHint = ({
                ...
                arrowup: "k",
                arrowright: "l",
            } as Record<string, string>)[released] ?? released;
    ...
    useEffect(() => {
        if (!hydrated || !tutorialEligible || !tutorialState.active)
            return;
        setTutorialState((state) => {
            const next = observeLearnTutorialSurface(state, "ai-writer", openSurfaces.aiWriter, pendingSurface.current === "ai-writer");
            if (next !== state)
                clearPendingSurface();
            return next;
        });
    ...
    useEffect(() => {
        ...
        const handleColumnCreated = (event: Event) => {
            if (!pendingColumnCreation.current)
                return;
            ...
            if (next === tutorialState || next.learnBoardId === null)
                return;
            pendingColumnCreation.current = false;
            setTutorialState(next);
            router.push(`/project?id=${next.learnBoardId}&tutorial=1${next.returnBoardId !== null
                ? `&tutorialReturn=${next.returnBoardId}`
                : ""}`);
        };
        window.addEventListener(LEARN_TUTORIAL_COLUMN_CREATED_EVENT, handleColumnCreated);
        return () => window.removeEventListener(LEARN_TUTORIAL_COLUMN_CREATED_EVENT, handleColumnCreated);
    }, [hydrated, router, tutorialEligible, tutorialState]);
    useEffect(() => {
        if (!hydrated || !tutorialEligible || !tutorialState.active)
            return;
        const handleColumnMove = (event: Event) => {
            if (!pendingColumnMove.current)
                return;
        ...
        return () => window.removeEventListener(LEARN_TUTORIAL_COLUMN_MOVE_EVENT, handleColumnMove);
    ...
    useEffect(() => {
        ...
        setTutorialState((state) => {
            const next = observeLearnTutorialInboxFocus(state, pending.key, pending.fromIndex, inboxFocus.currIdx);
            if (next !== state)
                clearPendingInboxMovement();
            return next;
        });
    ...
    useEffect(() => {
        if (!hydrated || !tutorialEligible || !tutorialState.active)
            return;
        setTutorialState((state) => {
            const next = observeLearnTutorialInboxTaskOpen(state, pathname ?? "", pendingInboxOpenTarget.current);
            if (next !== state)
                pendingInboxOpenTarget.current = null;
            return next;
        });
    ...
    useEffect(() => {
        ...
        const observeInbox = () => {
            ...
            const inboxLoaded = Boolean(document.querySelector('[data-tutorial-inbox-loaded="true"]'));
            setTutorialState((state) => {
                const navigated = observeLearnTutorialInboxNavigation(state, pathname ?? "", renderedNotificationIds);
                return observeLearnTutorialInboxZero(navigated, pathname ?? "", inboxLoaded, renderedNotificationIds);
            });
        };
    ...
    useEffect(() => {
        ...
        const handleInboxArchived = (event: Event) => {
            ...
            if (!archive)
                return;
            setTutorialState((state) => {
                const next = observeLearnTutorialInboxArchive(state, archive, pendingInboxArchiveTarget.current);
                if (next !== state)
                    pendingInboxArchiveTarget.current = null;
                return next;
            });
        };
        window.addEventListener(LEARN_TUTORIAL_INBOX_ARCHIVED_EVENT, handleInboxArchived);
        return () => window.removeEventListener(LEARN_TUTORIAL_INBOX_ARCHIVED_EVENT, handleInboxArchived);
    }, [hydrated, tutorialEligible, tutorialState.active]);
    useEffect(() => {
        if (!hydrated || !tutorialEligible || !tutorialState.active)
            return;
        ...
        window.addEventListener(LEARN_TUTORIAL_INBOX_ARCHIVE_FAILED_EVENT, handleInboxArchiveFailed);
        return () => window.removeEventListener(LEARN_TUTORIAL_INBOX_ARCHIVE_FAILED_EVENT, handleInboxArchiveFailed);
    }, [hydrated, tutorialEligible, tutorialState.active]);
    useEffect(() => {
        if (tutorialState.scene !== "goInbox") {
            setInboxNavigationStarted(false);
    ...
    useEffect(() => {
        const surface = tutorialState.scene === "assign"
            ? ("assignees" as const)
        ...
        const revealTimer = setTimeout(() => {
            ...
            if (!shortcutMatched)
                return;
            setTutorialState((state) => observeLearnTutorialSurface(state, surface, true, shortcutMatched));
            clearPendingSurface();
    ...
    useEffect(() => {
        const surface = tutorialState.scene === "dueDate"
            ? ("due-date" as const)
            : tutorialState.scene === "comment"
                ? ("comment-editor" as const)
                : null;
        const opened = surface === "due-date"
    ...
    useEffect(() => {
        const handleDueDateSaved = (event: Event) => {
            const savedTaskId = (event as CustomEvent<{
                taskId?: number;
            }>).detail
                ?.taskId;
            if (pendingSurface.current !== "due-date-committed" ||
                savedTaskId !== tutorialState.lastTaskId) {
                return;
            }
            setTutorialState((state) => observeLearnTutorialSurface(state, "due-date-committed", true, true));
            clearPendingSurface();
        };
        window.addEventListener(LEARN_TUTORIAL_DUE_DATE_SAVED_EVENT, handleDueDateSaved);
        return () => window.removeEventListener(LEARN_TUTORIAL_DUE_DATE_SAVED_EVENT, handleDueDateSaved);
    ...
    useEffect(() => {
        const handleCommentSaved = (event: Event) => {
            const savedTaskId = (event as CustomEvent<{
                ...
                ?.taskId;
            if (pendingSurface.current !== "comment-saved" ||
                savedTaskId !== tutorialState.lastTaskId) {
                return;
            }
            setTutorialState((state) => observeLearnTutorialSurface(state, "comment-saved", true, true));
            clearPendingSurface();
        };
        window.addEventListener(LEARN_TUTORIAL_COMMENT_SAVED_EVENT, handleCommentSaved);
        return () => window.removeEventListener(LEARN_TUTORIAL_COMMENT_SAVED_EVENT, handleCommentSaved);
    }, [clearPendingSurface, tutorialState.lastTaskId]);
    useEffect(() => {
        if (!hydrated ||
            !tutorialEligible ||
        ...
        const expectationGeneration = pendingSurfaceGeneration.current;
        const revealTimer = setTimeout(() => {
            const shortcutMatched = pendingSurface.current === "command-center" &&
                pendingSurfaceGeneration.current === expectationGeneration;
            if (!shortcutMatched)
                return;
            holdPendingSurface();
            resetShowCommands();
            const aiWriter = document.querySelector("#popover-wrapper-description textarea#htc");
            if (aiWriter) {
    ...
    useEffect(() => {
        if (commandHandoffGeneration === null)
            return;
        let cancelled = false;
        let focusTimer: ReturnType<typeof setTimeout> | null = null;
        let settleTimer: ReturnType<typeof setTimeout> | null = null;
        const focusAndVerify = () => {
            if (cancelled || focusTimer || settleTimer)
                return;
            const expectationStillCurrent = pendingSurface.current === "command-center" &&
            ...
            settleTimer = setTimeout(() => {
                settleTimer = null;
                const expectationRemainsCurrent = pendingSurface.current === "command-center" &&
                    pendingSurfaceGeneration.current === commandHandoffGeneration;
                if (expectationRemainsCurrent &&
                    anchor &&
                    document.activeElement === anchor) {
                    setTutorialState((state) => observeLearnTutorialSurface(state, "command-center", true, pendingSurface.current === "command-center"));
                    clearPendingSurface();
                    setCommandHandoffGeneration(null);
                    return;
                }
    ...
    useEffect(() => {
        ...
        const handleTaskMoved = (event: Event) => {
            ...
            const verifyRenderedDestination = () => {
                if (pendingBoardMove.current?.generation !== generation ||
                    pendingBoardMoveGeneration.current !== generation) {
                    return;
                }
                const renderedPosition = getRenderedBoardPosition(move.taskId);
                const expectedVerticalIndex = move.direction === "down"
                    ? pending.from.index + 1
                    : move.direction === "up"
                        ? pending.from.index - 1
                        : null;
                const renderedAtExpectedDestination = renderedPosition?.sectionId === move.toSectionId &&
                ...
                if (renderedAtExpectedDestination) {
                    ...
                    setTutorialState((state) => observeLearnTutorialBoardMove(state, verifiedMove, renderedPosition));
                    clearPendingBoardMove();
                ...
                attempts += 1;
                if (attempts < 200) {
                    pendingBoardMoveVerificationTimer.current = setTimeout(verifyRenderedDestination, 50);
                }
                else {
                    clearPendingBoardMove();
                    document.getElementById(`task-${move.taskId}`)?.focus();
        ...
        return () => window.removeEventListener(LEARN_TUTORIAL_TASK_MOVED_EVENT, handleTaskMoved);
    }, [
        clearPendingBoardMove,
        hydrated,
        tutorialEligible,
    ...
    useEffect(() => {
        ...
        const focusTutorialTask = () => {
            ...
            if (task && document.activeElement !== task)
                task.focus();
    ...
    useEffect(() => {
        if (!hydrated || !tutorialEligible || !tutorialState.active)
            return;
        setTutorialState((state) => {
            const next = observeLearnTutorialTaskFocus(state, focusedTaskId, pendingMovementKey.current);
            if (next !== state) {
                clearPendingMovement();
            }
            return next;
    ...
    useEffect(() => {
        ...
        setTutorialState((state) => {
            ...
            const escaped = observeLearnTutorialEscape(advanced, pathname ?? "", pendingEscapeFromDetail.current);
            if (escaped !== advanced)
    ...
    const activeHintCount = sceneName === "movement"
