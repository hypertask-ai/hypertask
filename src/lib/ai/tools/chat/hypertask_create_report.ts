...
export function createHypertaskCreateReportTool(context: ChatToolContext) {
  const { REPORT_BODY_MAX, REPORT_CAPABILITIES, REPORT_SLUG_RE, ReportValidationError, createReport, getReportUrl, sanitizeForJson, sendStatus, tool, user, validateProjectAccess, withToolErrors, z } = context;
  return tool({
