...
const TASK_SWIPE_RELEASE_THRESHOLD_RATIO = 0.35;
const TASK_SWIPE_EDGE_RESISTANCE = 0.2;
...
export const getTaskPlaylistBounds = (
  ...
  return {
    ...
    nextDisabled:
      !hasCurrent || currentIndex === (playlist?.length ?? 0) - 1,
  };
};
...
export const resistedTaskSwipeOffset = (
  ...
  return beyondStart || beyondEnd
    ? deltaX * TASK_SWIPE_EDGE_RESISTANCE
    : deltaX;
};
...
export const resolveTaskSwipeNavigation = (
  ...
  const threshold = Math.max(1, viewportWidth) * TASK_SWIPE_RELEASE_THRESHOLD_RATIO;
...
export const shouldIgnoreTaskSwipeStart = (
  ...
  for (
    ...
    if (element instanceof boundary.ownerDocument.defaultView!.HTMLElement) {
